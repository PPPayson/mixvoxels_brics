        ...
        for img_file in os.listdir(file_name):
            if img_file.endswith('.png'):
                ...
                if img_index >= start_frame and img_index < end_frame:
                    ...
                    tpth = os.path.join(tmp_dir, '%08d.png'%(img_index-start_frame))
                    cmd = f'cp {spth} {tpth}'
                    #print(cmd)
                    os.system(cmd)
        return tmp_dir
    else:
        os.system('ffmpeg -i {} -s {}x{} {}'.format(file_name, WIDTH, HEIGHT, os.path.join(tmp_dir, r'%08d.png')))
        return tmp_dir
...
def calc(directory_1, directory_2, flip_path, interval):
    ...
    for file1, file2 in ALL_PAIRS:
        ...
        if flip_path is not None:
            ...
            result = subprocess.check_output(['python', os.path.join(os.path.dirname(__file__), 'flip.py'), '--reference', file1, '--test', file2, '--no_error_map', '--no_error_map'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tmp_dir', type=str, default= os.path.join('/tmp/nerf_metric_temp'))
    parser.add_argument('--tmp_gt_dir', type=str, default= os.path.join('/tmp/nerf_metric_temp_gt'))
    ...
    directory_1 = to_directory(args.output, WIDTH, HEIGHT, args.tmp_dir, start_frame=args.start_frame, end_frame=args.end_frame)
